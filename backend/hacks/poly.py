from hacks.params import KYBER_N, KYBER_Q, KYBER_POLY_COMPRESSED_BYTES_768, \
...
from hacks.util import conditional_subq, cast_to_byte, cast_to_short, cast_to_int32, cast_to_long64, \
    cbd, montgomery_reduce, barrett_reduce
from hacks.prf import generate_prf_byte_array
from hacks.ntt import ntt, inv_ntt, base_multiplier, NTT_ZETAS
...
def compress_poly(poly_a, param_k):
    ...
    if (param_k == 2 or param_k == 3):
        ...
        for i in range(0, KYBER_N // 8):
            for j in range(0, 8):
                t[j] = cast_to_byte(
                    (((((poly_a[8 * i + j]) << 4) + (KYBER_Q // 2)) // (KYBER_Q)) & 15))
        ...
        for i in range(0, KYBER_N // 8):
            for j in range(0, 8):
                t[j] = cast_to_byte(
                    (((((poly_a[8 * i + j]) << 5) + (KYBER_Q // 2)) // (KYBER_Q)) & 31))
...
def decompress_poly(a, params_k):
    ...
    if (params_k == 2 or params_k == 3):
        for i in range(0, KYBER_N // 2):
            r[2 * i + 0] = cast_to_short(
                (((cast_to_int32(a[aa] & 0xFF) & 15) * KYBER_Q) + 8) >> 4)
            r[2 * i + 1] = cast_to_short(
                (((cast_to_int32(a[aa] & 0xFF) >> 4) * KYBER_Q) + 8) >> 4)
            aa = aa + 1
    else:
        t = [0 for x in range(0, 8)]  # long64
        for i in range(0, KYBER_N // 8):
            t[0] = cast_to_long64(cast_to_int32(a[aa + 0] & 0xFF) >> 0) & 0xFF
            t[1] = cast_to_long64(cast_to_byte((cast_to_int32(
                a[aa + 0] & 0xFF) >> 5)) | cast_to_byte(cast_to_int32(a[aa + 1] & 0xFF) << 3)) & 0xFF
            t[2] = cast_to_long64(cast_to_int32(a[aa + 1] & 0xFF) >> 2) & 0xFF
            t[3] = cast_to_long64(cast_to_byte((cast_to_int32(
                a[aa + 1] & 0xFF) >> 7)) | cast_to_byte(cast_to_int32(a[aa + 2] & 0xFF) << 1)) & 0xFF
            t[4] = cast_to_long64(cast_to_byte((cast_to_int32(
                a[aa + 2] & 0xFF) >> 4)) | cast_to_byte(cast_to_int32(a[aa + 3] & 0xFF) << 4)) & 0xFF
            t[5] = cast_to_long64(cast_to_int32(a[aa + 3] & 0xFF) >> 1) & 0xFF
            t[6] = cast_to_long64(cast_to_byte((cast_to_int32(
                a[aa + 3] & 0xFF) >> 6)) | cast_to_byte(cast_to_int32(a[aa + 4] & 0xFF) << 2)) & 0xFF
            t[7] = (cast_to_long64(cast_to_int32(a[aa + 4] & 0xFF) >> 3)) & 0xFF
            aa = aa + 5
            for j in range(0, 8):
                r[8 * i +
                    j] = cast_to_short(((cast_to_long64(t[j] & 31) * (KYBER_Q)) + 16) >> 5)
    return r
...
def poly_to_bytes(a):
    ...
    for i in range(0, KYBER_N // 2):
        ...
        r[3 * i + 1] = cast_to_byte(cast_to_int32(t0 >> 8)
                                    | cast_to_int32(t1 << 4))
...
def poly_from_bytes(a):
    """
    Deserialize a list of integers into a polynomial
    :param a: List of integers
    :return: List representing the polynomial
    """
    r = [0 for x in range(0, KYBER_POLY_BYTES)]
    for i in range(0, KYBER_N // 2):
        r[2 * i] = cast_to_short((((a[3 * i + 0] & 0xFF) >> 0)
                                 | ((a[3 * i + 1] & 0xFF) << 8)) & 0xFFF)
        r[2 * i + 1] = cast_to_short((((a[3 * i + 1] & 0xFF) >> 4)
                                     | ((a[3 * i + 2] & 0xFF) << 4)) & 0xFFF)
    return r
...
def poly_to_msg(a):
    ...
    for i in range(0, KYBER_N // 8):
        for j in range(0, 8):
            t = cast_to_int32(
                ((((cast_to_int32(a[8 * i + j])) << 1) + (KYBER_Q // 2)) // KYBER_Q) & 1)
...
def decompress_polyvec(a, params_k):
    ...
    """
    r = [[0 for x in range(0, KYBER_POLY_BYTES)]
         for y in range(0, params_k)]  # short array
    aa = 0  # int
    t = []  # int array
    if (params_k == 2 or params_k == 3):
        t = [0 for x in range(0, 4)]
